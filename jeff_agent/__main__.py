...
def main(host: str = "localhost", port: int = 10004) -> None:
    base_url = f"http://{host}:{port}"
    skill = AgentSkill(
        id="schedule-badminton",
        name="Jeff's Agent",
        description="Helps with finding Jeff's availability with badminton",
        tags=["scheduling", "badminton"],
        examples=["Are you free to play badminton on 2025-11-10?"],
    )
